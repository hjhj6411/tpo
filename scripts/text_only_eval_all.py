...
def build_prompt(query, profile, shuffled_options):
    """
    query: query_generator의 instance record
    profile: 해당 user의 profile
    shuffled_options: [(display_label, option_dict), ...] — ABCD 랜덤 셔플됨
    """
    attrs = profile.get("structured_attributes", {})

    gc_likes    = attrs.get("garment_category", {}).get("likes", [])
    gc_dislikes = attrs.get("garment_category", {}).get("dislikes", [])
    color_likes    = attrs.get("color", {}).get("likes", [])
    color_dislikes = attrs.get("color", {}).get("dislikes", [])
    pattern_likes    = attrs.get("pattern", {}).get("likes", [])
    pattern_dislikes = attrs.get("pattern", {}).get("dislikes", [])

    pref_lines = []
    if gc_likes:    pref_lines.append(f"  Garment likes:    {', '.join(gc_likes)}")
    if gc_dislikes: pref_lines.append(f"  Garment dislikes: {', '.join(gc_dislikes)}")
    if color_likes:    pref_lines.append(f"  Color likes:    {', '.join(color_likes)}")
    if color_dislikes: pref_lines.append(f"  Color dislikes: {', '.join(color_dislikes)}")
    if pattern_likes:    pref_lines.append(f"  Pattern likes:    {', '.join(pattern_likes)}")
    if pattern_dislikes: pref_lines.append(f"  Pattern dislikes: {', '.join(pattern_dislikes)}")

    pref_block = "\n".join(pref_lines) if pref_lines else "  (no preference info)"

    option_lines = []
    for label, opt in shuffled_options:
        a = opt["attributes"]
        line = (
            f"  {label}. garment={a.get('garment_category','?')} | "
            f"color={a.get('color','?')} | "
