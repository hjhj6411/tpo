...
extract_fsiglip2.py
-------------------
Encode the EXISTING img2dataset webdataset tars with zooclaw-FashionSigLIP2
(transformers, SigLIP2-base-patch16-384 fine-tune) and write L2-normalized
image embeddings to a NEW corpus folder. Reads clip_corpus read-only; never
modifies it. Mirror of fsiglip/extract_fsiglip.py with the open_clip loader
swapped for transformers (the new model is NOT an open_clip checkpoint).

NOTE: embeddings live in a DIFFERENT space from the Marqo-FashionSigLIP corpus
— never mix shards/indexes between fashionsiglip_corpus and
fashionsiglip2_corpus.
...
class TarDataset(Dataset):
    ...
    GPU isn't starved by PIL resize (384px inputs are heavier than 224px).
    Tar opened lazily per worker process."""
    def __init__(self, tar_path, processor):
...
def main():
    ...
    ap.add_argument("--batch", type=int, default=128,
                    help="384px inputs: heavier than the 224px v1 — default lower")
    ...
    print(f"[gpu{args.gpu}] {len(mine)}/{len(tars)} tars, batch={args.batch}, "
          f"workers={args.workers}, device={device}, model={args.model}")
    ...
    for tar_path in mine:
        ...
        ds = TarDataset(tar_path, processor)
        ...
        for batch in dl:
            ...
            with torch.no_grad(), torch.autocast(device_type="cuda", dtype=torch.float16):
                f = _pooler_output(model.get_image_features(pixel_values=xs))
