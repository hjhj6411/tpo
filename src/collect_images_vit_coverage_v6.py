#!/usr/bin/env python3
"""
collect_images_vit_coverage_v6.py — coverage-first TOP-1 + gender + dedup
----------------------------------------------------------------------------
ViT-L/14 CLIP twin of the FashionSigLIP coverage collector (identical logic;
only the recall backbone/port differs). Retrieval runs on ViT (clip-retrieval
back, :1234). Patch-coverage SCORING is model-agnostic and reuses the SigLIP
coverage server (serve_fsiglip_knn.py /patch-coverage, :1235): ViT finds the
candidates, SigLIP scores their coverage. Set --coverage-url empty to disable
coverage. Builds on v2 (top-1 + gender consistency + intra-set URL dedup), but fixes the
...
Usage:
python src/collect_images_vit_coverage_v6.py \
...
UA = {"User-Agent": "Mozilla/5.0 (compatible; POD-Bench-ViT-cov/6.0)"}
...
def main():
    ...
    ap.add_argument("--output", type=Path,
                    default=Path("data/images_vit_cov/collection_log.jsonl"))
    ap.add_argument("--image_root", type=Path,
                    default=Path("data/images_vit_cov"))
