#!/usr/bin/env python3
"""
collect_images_siglip_ensemble_v5.py  —  TOP-1 + gender + dedup + PROMPT ENSEMBLE
---------------------------------------------------------------------------------
Same as v2 (top-1 + gender consistency + intra-set URL dedup), but the retrieval
query is a FASHION TEMPLATE ENSEMBLE instead of one sentence. Each option's
(color, garment, pattern) attributes are filled into N short fashion templates;
the templates are sent to the server's /knn-service-ensemble endpoint, which
L2-normalizes each template embedding, AVERAGES them, re-normalizes, and searches.
This is the standard CLIP prompt-ensemble recipe (Radford et al. 2021): the
averaged text embedding is more robust than any single prompt, and short
templates avoid the 77-token blow-up of one long sentence.

Two quality constraints retained from v2:
...
Usage:
  python src/collect_images_siglip_top1_v2.py \
    --client-url http://127.0.0.1:1235/knn-service-ensemble \
...
# Hand-crafted FASHION prompt templates (CLIP prompt-ensemble). Short, caption-like,
# each well under the 77-token limit. {c}=color adjective (may be empty), {g}=garment,
# {pat}= an all-over pattern clause (empty for solid / color axis). The server encodes
# each, L2-normalizes, averages, re-normalizes, then searches.
_TEMPLATES_BASE = [
...
# extra templates that lean on the all-over nature of the pattern (used only when
# there's a non-solid pattern on the pattern axis)
_TEMPLATES_PATTERN = [
...
def build_templates(garment, target_color=None, target_pattern=None,
                    axis=None, fallback_query=None):
    """Return a LIST of short fashion prompts (the ensemble). Attributes are filled
    into each template; the embeddings are averaged server-side.

    - color adjective included when known.
    - all-over pattern phrasing ONLY on the pattern axis with a non-solid target;
      adds two pattern-emphasizing templates as well.
    - if no garment label, fall back to a couple of minimal wrappers on the raw query."""
    g = _clean(garment)
    ...
    for t in _TEMPLATES_BASE:
        prompts.append(t.format(c=c_adj, g=g, pat=pat_clause).replace("  ", " ").strip())
    if nonsolid:
        for t in _TEMPLATES_PATTERN:
            prompts.append(t.format(c=c_adj, g=g, patname=tp).replace("  ", " ").strip())
    # de-dup while preserving order (some templates collapse when clauses are empty)
    seen, uniq = set(), []
    for p in prompts:
        if p not in seen:
...
# -- retrieval + download --
def knn_search(client_url, queries, top_k=10, indice_name="pod_fashion"):
    """queries may be a single string OR a list of template strings.
    A list is sent to /knn-service-ensemble (embedding-average). A bare string
    uses whatever endpoint client_url already points at (back-compat)."""
    try:
        if isinstance(queries, (list, tuple)):
            payload = {"texts": list(queries), "modality": "image",
                       "num_images": top_k, "num_result_ids": top_k,
                       "indice_name": indice_name}
        else:
            payload = {"text": queries, "modality": "image",
                       "num_images": top_k, "num_result_ids": top_k,
                       "indice_name": indice_name}
        resp = _SESSION.post(client_url, json=payload, timeout=60)
        resp.raise_for_status()
        return resp.json()
    except Exception as e:
        n = len(queries) if isinstance(queries, (list, tuple)) else 1
        print(f"    [knn] query failed ({n} template(s)): {e}")
        return []
...
def resolve_option_top1(query, img_path, client_url, top_k, indice_name,
    ...
    `query` may be a single string or a list of ensemble templates."""
    cands = knn_search(client_url, query, top_k=top_k, indice_name=indice_name)
    # readable log fields for either input type
    q_log = query if isinstance(query, str) else " | ".join(query)
    templates = None if isinstance(query, str) else list(query)
    ...
    for rank, c in enumerate(cands):
        ...
        if not vlm_url:                # pure top-1 + dedup (no gender)
            return ({"image_path": str(img_path), "source": SOURCE_TAG,
                     "search_query": q_log, "templates": templates,
                     "rank_used": rank, "url": url,
    ...
    return ({"image_path": None, "source": "FAILED", "search_query": q_log}, None, None)
...
def collect_for_plan(plan, image_root, client_url, top_k, indice_name,
    ...
    for k in "ABCD":
        ...
        # build a FASHION TEMPLATE ENSEMBLE from the option's attributes:
        #   - garment label + color adjective in every template
        #   - all-over pattern templates only on pattern axis + non-solid
        # the list is averaged server-side (/knn-service-ensemble)
        target_color = attrs.get("color")
        target_pattern = attrs.get("pattern")
        templates = build_templates(attrs.get("garment_category"),
        ...
        res, seen, url = resolve_option_top1(
            templates, img_path, client_url, top_k, indice_name,
            vlm_url, vlm_model, target_gender, used_urls)
...
def main():
    ...
    ap.add_argument("--client-url", default="http://127.0.0.1:1235/knn-service-ensemble")
