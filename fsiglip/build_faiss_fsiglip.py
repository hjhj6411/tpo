#!/usr/bin/env python3
"""
build_faiss_fsiglip.py
----------------------
Concatenate per-shard FashionSigLIP embeddings into one IndexFlatIP (cosine =
...
the KNN server returns. Joins shard `key` to metadata parquet for `url`.
  ...
  python build_faiss_fsiglip.py
Outputs in fashionsiglip_corpus/:
...
def main():
    ...
    assert npys, f"no shard .npy in {shard_dir} — run extract_fsiglip.py first"
    ...
    json.dump({"model": "Marqo/marqo-fashionSigLIP", "dim": DIM,
