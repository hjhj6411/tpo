#!/usr/bin/env python3
# NOT superseded — this file is a LIVE DEPENDENCY.
# `availability_audit/screen_sam3.py` imports its scorers from here
# (query_knn, score_pattern_patches, score_color_patches, score_garment_vlm,
# TEXT_QUERY_GARMENT_VOCAB, PATTERN_VOCAB, ...), which makes this module the
# scoring implementation behind `annotation/attribute_library.json`. Do not
# delete or archive it, and keep its vocabulary in step with
# fsiglip/collector_sam3.py — the garment vocabulary in BOTH is now derived
# from configs/config.py for exactly that reason.
#
...
# 터미널 3: 수집 실행
conda activate sam3
python fsiglip/collect_img_sam3.py \
  ...
  --output-root data/retrieval/sam3vlmfix_benchmark \
  --pattern-short-side-tiles 5 \
  --color-short-side-tiles 15 \
  --patch-mask-fill local_mean \
  --options A,B,C,D --device cuda --force
"""
...
def task_dirs(args: argparse.Namespace, task: OptionTask) -> tuple[Path, Path, Path]:
    ...
    return (
        ...
        args.work_root / "topk_patches" / q / opt,
        args.output_root / "benchmark_crops" / q,
    )


def save_benchmark_crop(records: list[dict[str, Any]], task: OptionTask, args: argparse.Namespace) -> Path | None:
    """Keep only the reranked top-1 crop for one query-option."""
    ...
    for rec in rows:
        src_path = rec.get("crop_path")
        if not src_path or not Path(src_path).exists():
            continue

        src = Path(src_path)
        ext = src.suffix or ".jpg"
        name = f"{task.option_label}{ext.lower()}"
        dst = rdir / name
        shutil.copy2(src, dst)
        rec["benchmark_crop_path"] = str(dst)
...
def main() -> None:
    ...
    if args.output_root.exists():
        if args.force:
            ...
            raise SystemExit(
                ...
                "Use a new --output-root or pass --force to replace it."
            )
    ...
    for i, task in enumerate(tasks, start=1):
        ...
        # Downloads, masks, patches, and overlays are still generated exactly as
        # before because they are inputs to the scoring logic. Keep them in a
        # per-task temporary directory so benchmark output contains one crop.
        with tempfile.TemporaryDirectory(prefix="pod_benchmark_crop_") as work_dir:
            args.work_root = Path(work_dir)
    ...
    print()
    print("Done.")
    print(f"benchmark crops: {args.output_root / 'benchmark_crops'}")
