...
# fsiglip/collect_topk_sam3_fsiglip_patch_rank_vlm_garment_axis_patches.py.
...
# Retrieval and pattern/color patch embeddings use ViT-L/14 instead of
# FashionSigLIP. Pattern and color scoring use separate patch grids because patterns need
...
# 터미널 3: 수집 실행
conda activate sam3
python vit/collect_topk_sam3_vit_l14_patch_rank_vlm_garment_axis_patches.py \
  ...
  --output-root data/retrieval/sam3_vit_l14_patch_rank \
...
_VIT_PROCESSOR = None
_VIT_DEVICE = None
_VIT_TEXT_CACHE: dict[tuple[str, ...], torch.Tensor] = {}
...
def score_image_files(
    ...
    del session, args
    if _VIT_MODEL is None or _VIT_PROCESSOR is None or _VIT_DEVICE is None:
        raise RuntimeError("ViT-L/14 model is not loaded")

    keys = list(texts)
    prompts = tuple(str(texts[key]) for key in keys)
    text_features = _VIT_TEXT_CACHE.get(prompts)

    def amp_context():
        return (
            torch.autocast("cuda", dtype=torch.float16)
            if str(_VIT_DEVICE).startswith("cuda") and torch.cuda.is_available()
            else nullcontext()
        )

    if text_features is None:
        inputs = _VIT_PROCESSOR(text=list(prompts), padding=True, return_tensors="pt")
        inputs = {key: value.to(_VIT_DEVICE) for key, value in inputs.items()}
        with torch.inference_mode(), amp_context():
            encoded = _VIT_MODEL.get_text_features(**inputs)
            text_features = vit_feature_tensor(encoded)
            text_features = torch.nn.functional.normalize(text_features.float(), dim=-1)
        _VIT_TEXT_CACHE[prompts] = text_features

    out: list[dict[str, float]] = [{} for _ in image_paths]
    ...
    for i in range(0, len(image_paths), batch_size):
        ...
        inputs = _VIT_PROCESSOR(images=images, return_tensors="pt")
        pixel_values = inputs["pixel_values"].to(_VIT_DEVICE)
        with torch.inference_mode(), amp_context():
            encoded = _VIT_MODEL.get_image_features(pixel_values=pixel_values)
            image_features = vit_feature_tensor(encoded)
            image_features = torch.nn.functional.normalize(image_features.float(), dim=-1)
            similarities = image_features @ text_features.T

        for row_idx, scores in zip(valid_indices, similarities.detach().cpu().tolist()):
            out[row_idx] = {key: float(value) for key, value in zip(keys, scores)}

    return out


def vit_feature_tensor(value: Any) -> torch.Tensor:
    ...
    if isinstance(value, (tuple, list)) and value and torch.is_tensor(value[0]):
        return value[0]
    raise TypeError(f"unexpected ViT feature output: {type(value).__name__}")


def load_vit_model(model_name: str, device: str) -> None:
    global _VIT_MODEL, _VIT_PROCESSOR, _VIT_DEVICE
    from transformers import CLIPModel, CLIPProcessor

    _VIT_DEVICE = torch.device(device)
    _VIT_PROCESSOR = CLIPProcessor.from_pretrained(model_name)
    _VIT_MODEL = CLIPModel.from_pretrained(model_name).to(_VIT_DEVICE).eval()
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--client-url", default="http://127.0.0.1:1234/knn-service")
    p.add_argument("--score-url", default="", help="Accepted for CLI compatibility; ViT patch scoring is local.")
    p.add_argument("--index-name", default="pod_fashion")
    p.add_argument("--embedding-model", default=VIT_MODEL_NAME)
    p.add_argument("--output-root", type=Path, default=Path("data/retrieval/sam3_vit_l14_patch_rank"))
...
def main() -> None:
    ...
    print("=" * 96)
    print("ViT-L/14 top-k -> SAM3 garment mask -> ViT-L/14 patch/global ranking")
    print("=" * 96)
    ...
    print(f"loading ViT embeddings: {args.embedding_model}...")
    load_vit_model(args.embedding_model, args.device)
